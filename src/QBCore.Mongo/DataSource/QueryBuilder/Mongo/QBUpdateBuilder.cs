...
internal sealed class QBUpdateBuilder<TDoc, TDto> : IQBUpdateBuilder<TDoc, TDto>, IQBMongoUpdateBuilder<TDoc, TDto>, ICloneable
{
	public QueryBuilderTypes QueryBuilderType => QueryBuilderTypes.Update;
	...
	public Expression<Func<TDoc, object?>>? IdField
	{
		...
		{
			if (_idField != null)
				throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}': option '{nameof(IdField)}' is already set.");
			_idField = value;
		}
	}
	public Expression<Func<TDoc, object?>>? DateUpdateField
	{
		...
		set
		{
			if (_dateUpdateField != null)
				throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}': option '{nameof(DateUpdateField)}' is already set.");
	...
	public Expression<Func<TDoc, object?>>? DateModifyField
	{
		...
		set
		{
			if (_dateModifyField != null)
				throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}': option '{nameof(DateModifyField)}' is already set.");
	...
	public QBUpdateBuilder()
	{
		...
		_dateUpdateField =
			typeof(TDoc).GetProperties().Where(x => x.IsDefined(typeof(DeUpdatedAttribute), true)).FirstOrDefault()?.ToMemberExpression<TDoc>()
			?? typeof(TDoc).GetFields().Where(x => x.IsDefined(typeof(DeUpdatedAttribute), true)).FirstOrDefault()?.ToMemberExpression<TDoc>();
	...
	public QBUpdateBuilder(QBUpdateBuilder<TDoc, TDto> other)
	{
	...
	public object Clone() => new QBUpdateBuilder<TDoc, TDto>(this);
	...
	private void NormalizeUpdate()
	{
		...
		{
			throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}'.");
		}
		...
		for (int i = 1, j = 0; i < containers.Count - j; i++)
		{
			...
			if (temp.ContainerOperation == ContainerOperations.LeftJoin || temp.ContainerOperation == ContainerOperations.Join)
			{
				...
				{
					throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}': JOIN (LEFT JOIN) has to have at least one connect condition on a field.");
				}
			}
			else if (temp.ContainerOperation == ContainerOperations.CrossJoin)
			{
				if (connects.Any(x => x.IsConnectOnField && x.Alias == temp.Alias))
				{
					throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}': CROSS JOIN cannot have connection conditions on fields.");
				}
	...
	private QBUpdateBuilder<TDoc, TDto> AddContainer(
		Type documentType,
		string alias,
		string dbSideName,
		...
		ContainerOperations containerOperation)
	{
		...
		{
			throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}': another initial container has already been added before.");
		}
		if (alias.Contains('.'))
		{
			throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}': container alias '{alias}' cannot contain a period character '.'.");
		}
		if (_containers.Any(x => x.Alias == alias))
		{
			throw new InvalidOperationException($"Incorrect definition of update query builder '{typeof(TDto).ToPretty()}': initial container '{alias}' has already been added before.");
		}
	...
	private QBUpdateBuilder<TDoc, TDto> AddParameter(string name, Type underlyingType, bool isNullable, System.Data.ParameterDirection direction)
	{
		if (_parameters == null)
		{
		...
		if (param != null)
		{
			...
			{
				throw new InvalidOperationException($"Incorrect parameter definition of update query builder '{typeof(TDto).ToPretty()}': parameter '{name}' has already been added before with different properties");
			}
		}
		else
		{
