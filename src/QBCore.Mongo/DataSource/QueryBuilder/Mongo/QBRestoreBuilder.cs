...
internal sealed class QBRestoreBuilder<TDoc, TDto> : IQBRestoreBuilder<TDoc, TDto>, IQBMongoRestoreBuilder<TDoc, TDto>, ICloneable
{
	...
	public Expression<Func<TDoc, object?>>? IdField
	{
		...
		{
			if (_idField != null)
				throw new InvalidOperationException($"Incorrect definition of restore query builder '{typeof(TDto).ToPretty()}': option '{nameof(IdField)}' is already set.");
			_idField = value;
		}
	}
	public Expression<Func<TDoc, object?>>? DateDeleteField
	{
		...
		set
		{
			if (_dateDeleteField != null)
				throw new InvalidOperationException($"Incorrect definition of restore query builder '{typeof(TDto).ToPretty()}': option '{nameof(DateDeleteField)}' is already set.");
	...
	public QBRestoreBuilder(QBRestoreBuilder<TDoc, TDto> other)
	{
	...
	public object Clone() => new QBRestoreBuilder<TDoc, TDto>(this);
	...
	private void NormalizeInternal()
	{
		...
		{
			throw new InvalidOperationException($"Incorrect definition of restore query builder '{typeof(TDto).ToPretty()}'.");
		}
		...
		for (int i = 1, j = 0; i < containers.Count - j; i++)
		{
			...
			if (temp.ContainerOperation == ContainerOperations.LeftJoin || temp.ContainerOperation == ContainerOperations.Join)
			{
				...
				{
					throw new InvalidOperationException($"Incorrect definition of restore query builder '{typeof(TDto).ToPretty()}': JOIN (LEFT JOIN) has to have at least one connect condition on a field.");
				}
			}
			else if (temp.ContainerOperation == ContainerOperations.CrossJoin)
			{
				if (connects.Any(x => x.IsConnectOnField && x.Alias == temp.Alias))
				{
					throw new InvalidOperationException($"Incorrect definition of restore query builder '{typeof(TDto).ToPretty()}': CROSS JOIN cannot have connection conditions on fields.");
				}
		...
		ContainerOperations containerOperation)
	{
		...
		{
			throw new InvalidOperationException($"Incorrect definition of restore query builder '{typeof(TDto).ToPretty()}': another initial container has already been added before.");
		}
		if (alias.Contains('.'))
		{
		...
		{
			throw new InvalidOperationException($"Incorrect definition of restore query builder '{typeof(TDto).ToPretty()}': initial container '{alias}' has already been added before.");
		}
	...
	private QBRestoreBuilder<TDoc, TDto> AddParameter(string name, Type underlyingType, bool isNullable, System.Data.ParameterDirection direction)
	{
		if (_parameters == null)
		{
		...
		if (param != null)
		{
			...
			{
				throw new InvalidOperationException($"Incorrect parameter definition of restore query builder '{typeof(TDto).ToPretty()}': parameter '{name}' has already been added before with different properties");
			}
		}
		else
		{
