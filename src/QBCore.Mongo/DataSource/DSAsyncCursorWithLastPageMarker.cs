...
internal sealed class DSAsyncCursorWithLastPageMarker<T> : IDSAsyncCursor<T>
{
	...
	public DSAsyncCursorWithLastPageMarker(IAsyncCursor<T> cursor, int take, CancellationToken cancellationToken)
	{
		_cursor = cursor;
	...
	public bool MoveNext(CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		{
			var reverseCounter = _reverseCounter -= _cursor.Current.Count();
