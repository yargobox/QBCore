	...
	where TDocument : class
{
	...
	public SoftDelQueryBuilder(SoftDelQBBuilder<TDocument, TDelete> building, IDataContext dataContext) : base(building, dataContext)
	{
		building.Normalize();
	}
	...
	public async Task DeleteAsync(object id, TDelete? document = default(TDelete?), DataSourceDeleteOptions? options = null, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		{
			throw new NotSupportedException($"PostgreSQL soft delete query builder does not support an operation like '{top.ContainerOperation.ToString()}'.");
		}
		...
		try
		{
			//((SqlDocumentInfo?)Builder.DocumentInfo).EntityType.GetSchemaQualifiedTableName()
