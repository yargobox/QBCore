...
public class QBSortOrder
{
	...
	internal class EntityTypeConfiguration : PluralNamingConfiguration<QBSortOrder>
	{
		public override void Configure(EntityTypeBuilder<QBSortOrder> builder)
		{
			builder
				.ToTable(ObjectName, SchemaName)
				.HasKey(x => x.QBSortOrderId);

			builder
				.HasOne(x => x.QueryBuilder)
				.WithMany(x => x.QBSortOrders)
				.IsRequired()
